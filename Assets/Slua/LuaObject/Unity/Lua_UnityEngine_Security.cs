...
public class Lua_UnityEngine_Security : LuaObject {
	...
	static public int LoadAndVerifyAssembly_s(IntPtr l) {
		try {
			...
			pushValue(l,false);
			LuaDLL.lua_pushstring(l,"No matched override function LoadAndVerifyAssembly to call");
			return 2;
		}
		catch(Exception e) {
			return error(l,e);
	...
	static public int PrefetchSocketPolicy_s(IntPtr l) {
		try {
			...
			pushValue(l,false);
			LuaDLL.lua_pushstring(l,"No matched override function PrefetchSocketPolicy to call");
			return 2;
		}
		catch(Exception e) {
			return error(l,e);
